...
import { View, StyleSheet, Text, TouchableWithoutFeedback, Modal, TouchableOpacity, TextInput, ScrollView, ImageBackground } from "react-native";
...
export default function Messages(){
                ...
                if (messageData[sender][i].sender === jwtDecode(jwt).sub) {
                    messagesBySender.push(
                        <View key={messageData[sender][i].id} style={styles.messageSent}>
                            <Text style={styles.messageText}>{messageData[sender][i].content}</Text>
                            <Text style={styles.messageDate}>{getTime(messageData[sender][i].dateTime)}</Text>
                        </View>
                    );
                } else {
                    messagesBySender.push(
                        <View key={messageData[sender][i].id} style={styles.messageReceived}>
                            <Text style={styles.messageText}>{messageData[sender][i].content}</Text>
                            <Text style={styles.messageDate}>{getTime(messageData[sender][i].dateTime)}</Text>
                        </View>
                    );
                    ...
                    <Modal key={`${sender}m`} visible={true} animationType="slide">
                        ...
                        <View style={styles.messageView}>
                            <ScrollView>
                                <View key={`${sender}m`} id={sender} style={styles.messageContainer}>
                                    {messagesBySender}
                                </View>
                            </ScrollView>
        ...
        <TouchableWithoutFeedback key={item} onPress={() => changeConv(item)}>
          <View style={styles.conversation}>
            <View style={styles.conversationTop}>
                ...
                {messageData[item] &&
                    <Text>{getTime(messageData[item][messageData[item].length - 1].dateTime)}</Text>
                }
            </View>
            {messageData[item] &&
                <Text>{messageData[item][messageData[item].length - 1].content}</Text>
            }
          </View>
    ...
    return (
        <ImageBackground source={require("../components/Background.jpeg")} style={styles.backgroundImage}>
            <View>
                <Text style={styles.title}>Click on a conversation to start chatting</Text>
                <View>
                    {conversations}
                </View>
                <View>
